...
public class DB_CentroResido {
    ...
    public void inserirHistorico(Module_CentroRecido inventario) {
        String sql = "INSERT INTO centroResido (cod_dep, idProdut, tipo_equipamento, marca, modelo, num_serie, data_entrada, operador, funcao, local_sala, departamento, obs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection connection = dbConnection.connect();
            ...
            stmt.setString(6, inventario.getNumSerie());

            // Verifica se a data de entrada não é nula antes de definir
            if (inventario.getDataEntrada() != null) {
                stmt.setDate(7, java.sql.Date.valueOf(inventario.getDataEntrada()));
            } else {
                stmt.setDate(7, null);
            }
            ...
            stmt.setString(10, inventario.getLocalSala());
    ...
    public void atualizarCentroResido(Module_CentroRecido inventario) {
        String sql = "UPDATE centroResido SET idProdut = ?, tipo_equipamento = ?, marca = ?, modelo = ?, num_serie = ?, data_entrada = ?, operador = ?, funcao = ?, local_sala = ?, departamento = ?, obs = ? WHERE cod_dep = ?";
        try (Connection connection = dbConnection.connect();
             PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, inventario.getIdProdut()); // Adicionando idProdut
            ...
            stmt.setString(12, inventario.getCodDep());
